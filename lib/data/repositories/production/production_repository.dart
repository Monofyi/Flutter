...
class ProductionRepository {
  Future<List<BuyerModel>> newProduction() async {
    ...
    final suppliers = await get(
      Uri.parse('http://65.1.236.26:8000/start_production/'),
      headers: {
  ...
  Future<List<BuyerModel>> fetchOnGoingProductions() async {
    final token = await const AccountRepository().getToken();

    final suppliers = await get(
      Uri.parse('http://65.1.236.26:8000/view_ongoing_production/'),
      headers: {
        "Content-Type": "application/json",
        'Authorization': 'Token $token',
      },
